  ...
  return (
    <>
      <div
        ...
        style={{
          opacity: openShoppingCartMenu ? 1 : 0,
          pointerEvents: openShoppingCartMenu ? "auto" : "none",
        }}
        onClick={onClose}
      ></div>
      <section
        className={`${
          openShoppingCartMenu ? "block" : "hidden"
        } w-[90%] h-full overflow-auto bg-white fixed top-0 right-0 z-20 py-5 shadow-lg  flex flex-col gap-10 md:w-[50%] lg:w-[30%] `}
      >
        <div>
          <div className="flex justify-between items-center px-5">
            <h4 className="font-bold text-black/50 text-3xl">Mi carrito</h4>
            <img
              onClick={onClose}
              src={iconoCerrar}
        ...
        <section className="w-full flex flex-col justify-center items-center gap-10">
          {ShoppingCart.map((productsCart) => (
            ...
            >
              ...
              <div className="flex flex-col justify-center items-center gap-2 sm:flex-row sm:gap-10">
                <p className="font-bold text-black/50">{productsCart.name}</p>
                <p>{formatCurrencyCOP(productsCart.price)}</p>
                <p>{productsCart.quantity}</p>
          ...
          <div className="flex justify-around items-center w-full ">
            <h5 className="font-bold text-black/50 text-3xl">TOTAL</h5>
